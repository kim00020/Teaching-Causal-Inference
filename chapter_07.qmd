...
$$Y_i(t) \;=\; \beta t + \varepsilon_i.$$ {#eq-lab-po}
...
**Why IV restores identification.** The instrument $Z_i$ is drawn independently of $U_i$ and $\xi_i$, so $Z_i \indep U_i$ implies $Z_i \indep \varepsilon_i$ --- exogeneity, satisfied by construction. The instrument does not appear in $Y_i(t)$ (@eq-lab-po), which is the exclusion restriction: $Y_i(t, z) = Y_i(t)$ for all $z$. And $Z_i$ moves $T_i$ through $\pi$, which is relevance. All three IV assumptions hold exactly by construction.
