...
**DGP A: constant treatment effects.** Every unit has the same effect $\tau_i = 0.5$. Then the reduced form is $P(\mathrm{co}) \cdot \tau_{\mathrm{co}} = 0.2 \times 0.5 = 0.1$ ✓, and $\mathrm{ATE} = 0.5$.

**DGP B: heterogeneous effects with monotonicity.** Effects differ by type: $\tau_{\mathrm{co}} = 0.5$, $\tau_{\mathrm{at}} = 0$, $\tau_{\mathrm{nt}} = 0$. The reduced form is again $0.2 \times 0.5 = 0.1$ ✓, but
...
| Estimand depends on instrument? | No (same $\beta$ regardless of $Z$) | Yes (different $Z$ ⟹ different compliers ⟹ different LATE) |
