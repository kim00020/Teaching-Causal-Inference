...
The asymptotic variance equals the semiparametric efficiency bound; $\hat\tau_{\mathrm{DML}}$ is asymptotically efficient whenever @eq-w12-product-rate holds.
:::
...
by the product-rate condition @eq-w12-product-rate. Hence $\sqrt{n}\,B_n^{(k)} = o_p(1)$.
...
3. **Select nuisance learners.** Choose flexible learners for $\mu_0(x)$, $\mu_1(x)$, and $\pi(x)$. Verify informally that the product-rate condition @eq-w12-product-rate is plausible.
