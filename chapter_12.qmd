...
Differentiation and expectation are interchanged under standard bounded-convergence arguments; the LIE is applied by conditioning on $X$.

**Perturbation in $\pi$.** Replace $\pi$ by $\pi + rh$ and differentiate. The only $\pi$-dependent terms are the two IPW residual terms:
$$\left.\frac{\partial}{\partial r}\right|_{r=0} = \E\!\left[-\frac{T\,h(X)}{\pi(X)^2}\{Y-\mu_1(X)\} - \frac{(1-T)\,h(X)}{(1-\pi(X))^2}\{Y-\mu_0(X)\}\right].$$
Conditioning on $X$ and using $\E(T \mid X) = \pi(X)$: $\E[T\{Y-\mu_1(X)\} \mid X] = \pi(X)\mu_1(X) - \mu_1(X)\pi(X) = 0$, and similarly for the control term. Both conditional expectations vanish.

**Perturbation in $\mu_1$.** Replace $\mu_1$ by $\mu_1 + rg$. Differentiating:
$$\E\!\left[-\frac{T}{\pi(X)}\,g(X) + g(X)\right] = \E\!\left[g(X)\left(1 - \frac{T}{\pi(X)}\right)\right].$$
Conditioning on $X$: $\E[1 - T/\pi(X) \mid X] = 1 - \pi(X)/\pi(X) = 0$.

**Perturbation in $\mu_0$.** Replace $\mu_0$ by $\mu_0 + r\ell$. Differentiating:
$$\E\!\left[\ell(X)\left(\frac{1-T}{1-\pi(X)} - 1\right)\right].$$
Conditioning on $X$: $\E[(1-T)/(1-\pi(X)) - 1 \mid X] = (1-\pi(X))/(1-\pi(X)) - 1 = 0$. $\square$
:::

::: {.callout-note icon=false}
## Remark: What the Proof Reveals

Each of the three derivative calculations reduces to a single key fact: the conditional mean of the IPW weight equals one. Specifically, $\E(T/\pi(X) \mid X) = 1$ and $\E((1-T)/(1-\pi(X)) \mid X) = 1$ are immediate consequences of $\E(T \mid X) = \pi(X)$, which is the engine behind both the balancing theorem and the IPW identification formula (Chapter 6). Neyman orthogonality of the ATE score is therefore a direct consequence of how the propensity score is defined, not an additional requirement imposed on it.
:::
