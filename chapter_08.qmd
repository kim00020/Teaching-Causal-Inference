...
$$Y = f_Y(T,\, M,\, \mathbf{X},\, U,\, \varepsilon_Y),$$ {#eq-sem-y}
...
Condition (F2) is the front-door analogue of the mediator overlap (P2): it ensures that $P(y \mid m, t')$ is empirically defined at every $(m, t')$ cell that receives positive weight in @eq-frontdoor.
...
(c) Add a direct edge $T \to Y$ to the graph. Which front-door condition is violated? Does @eq-frontdoor still hold?
...
(c) Explain why the inner average uses the *covariate-conditional* distribution $P(t' \mid \mathbf{x})$ rather than the marginal $P(t')$ used in @eq-frontdoor.
