...
**Convergence.** The three frameworks are closely related and often intertranslatable, but exact equivalence requires additional assumptions about the causal model; see @wang2026tale for a recent side-by-side comparison. These notes work mainly in an NPSEM-IE setting, where translation between the languages is especially clean. Under weaker counterfactual models --- for example, Robins's FFRCISTG [@robins1986new] --- some cross-world quantities that an NPSEM-IE identifies are no longer well-defined, so equivalence across the three languages is more qualified. The historical differences between the three schools are primarily in notation, native strengths, and disciplinary culture, with substantive differences appearing mainly in how much cross-world structure each framework commits to.
